...
data <- data.frame(drought, grazing, drought_value, grazing_value, test_variance)

#sample plot - color by grazing treatment
ggplot(data=data, aes(x=as.factor(drought), y=drought_value, fill=grazing)) +
...
ggplot(data=data, aes(x=grazing, y=grazing_value, fill=as.factor(drought))) +
  geom_bar(stat='identity', position=position_dodge(), color='black') +
  geom_errorbar(aes(ymin=grazing_value-(test_variance/5), ymax=grazing_value+(test_variance/5)), position=position_dodge(0.9), width=0.2) +
